...
$k=$j+15;
$uid=$_COOKIE['iunv_id'];
$se = mysql_query("SELECT * FROM story_connects WHERE uid='$uid'");
$num=mysql_num_rows($se);
$s = mysql_query("SELECT * FROM story_connects WHERE uid='$uid' ORDER BY  `story_connects`.`time` DESC LIMIT $j, 15");
while($pt= mysql_fetch_assoc($s)){
$sid=$pt['sid'];
...
if($num>$k){
echo '<div id="moreFeed" onmouseover=moreFeed("'.$k.'")><center>More</center></div>';
}
